...
// Get site issues summary
export async function getSiteIssuesSummary(supabaseAdmin, dateRange, startDate, endDate) {
  try {
    // Count total open issues
    const { count: openIssuesCount, error: openError } = await supabaseAdmin
      .from('site_issues')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'open')
      .eq('deleted', false);
    ...
    if (openError) {
      console.error("Error getting open issues count:", openError);
      return { openIssues: 0, criticalIssues: 0, resolvedRecently: 0 };
    }
    
    // Count critical issues
    const { count: criticalIssuesCount, error: criticalError } = await supabaseAdmin
      .from('site_issues')
      .select('*', { count: 'exact', head: true })
    ...
    if (criticalError) {
      console.error("Error getting critical issues count:", criticalError);
      return { openIssues: openIssuesCount || 0, criticalIssues: 0, resolvedRecently: 0 };
    }
    
    // Count recently resolved issues
    let resolvedRecentlyQuery;
    const now = new Date();
    
    if (startDate && endDate) {
      resolvedRecentlyQuery = supabaseAdmin
        .from('site_issues')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'resolved')
        .eq('deleted', false)
      ...
      switch (dateRange) {
        ...
        case 'month':
          ...
          break;
        case 'year':
          timeAgo.setDate(now.getDate() - 365);
          break;
        default:
          timeAgo.setDate(now.getDate() - 7);
      }
    ...
    const { count: resolvedRecentlyCount, error: resolvedError } = await resolvedRecentlyQuery;
    
    if (resolvedError) {
      console.error("Error getting recently resolved issues count:", resolvedError);
      return { 
    ...
    return {
      openIssues: openIssuesCount || 0,
      criticalIssues: criticalIssuesCount || 0,
      resolvedRecently: resolvedRecentlyCount || 0
    };
  } catch (error) {
    console.error("Error getting site issues summary:", error);
    return { openIssues: 0, criticalIssues: 0, resolvedRecently: 0 };
  }
}
